    ...
    for symbol in df.loc[:, "Code"]:
        ...
        for i in range(1, len(df['Date']) - 1):
            ...
            if check_positive(df['macd_hist'][i]):
                ...
                bearish_macd.append([i, df['Date'][i], df['macd_hist'][i], df['High'][i], df['Close'][i]])
            else:
                # Adding all non-positive values to bullish
                bullish_macd.append([i, df['Date'][i], df['macd_hist'][i], df['Low'][i], df['Close'][i]])

        bullish_macd_df = pd.DataFrame(bullish_macd, columns=['Index', 'Date', 'macd_hist', 'Low', 'Close'])
        bearish_macd_df = pd.DataFrame(bearish_macd, columns=['Index', 'Date', 'macd_hist', 'High', 'Close'])
